...
fn progress_grow(
    time: Res<Time>,
    mut query: Query<(&mut Growable, &mut Handle<Image>, &mut Draggable, &Child, &mut Hitbox, &mut Sprite)>
) {
    for (mut growable, mut image, mut draggable, child, mut hitbox, mut sprite) in &mut query {
